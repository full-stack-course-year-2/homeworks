...
result = people.reduce((acc,value)=>{
    acc = acc + Number(value.salary);
    return acc;
},0);
...
const oldPeopleThan30Age = people.map((item)=>{
    const yil = item.DOB.split('/')[2];
    const simdikiYil = new Date().getFullYear();
    const age = simdikiYil - yil;
    return {
        age,
        ...item,
    }
});
const sonuc = oldPeopleThan30Age.filter((item)=>item.age>30);
console.log(sonuc);


// 3) Kişilerin tam adının bir listesini alın (ad ve soyadı).
people.forEach(insan => console.log(insan.firstName, insan.lastName))



// 4) Küçükten büyüğe doğru sıralanmış dizideki kişilerin bir listesini alın.
const siralama = people.map((item) => {
    const yil = item.DOB.split("/")[2];
