...
const siparisListesi = orders.filter(item => 
    (item.orderId == 234 && item.delivered  == false)).map(item => {
    return {
...
const siparisToplam = orders.map(item => {
    return {
        ...item,
        Siparis_Toplami: item.items.reduce((acc, item) => {
            acc += item.ucret;
            return acc;
        }, 0)
    }
...
sonuc = orders.some(siparis => siparis.items.some(item => item.productId == "123"));
console.log(sonuc);