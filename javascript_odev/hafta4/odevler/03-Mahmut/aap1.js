...
sonuc = characters.map((item) => item.height);
console.log(sonuc);
//3. Yalnızca ad ve yükseklik özelliklerine sahip nesne dizisini alın
sonuc = characters.map((item) => ({ name: item.name, height: item.height }));
console.log(sonuc);
...
sonuc = characters.some(
  (item) => item.eye_color == "blue" && item.gender == "male"
);
console.log(sonuc);
//3. 210'dan uzun en az bir karakter var mı?.
sonuc = characters.some((item) => item.height > 210 && item.gender == "male");
console.log(sonuc);
